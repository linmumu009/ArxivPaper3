...
function $(selector: string) {
  return addon.data.prefs?.window?.document.getElementById(selector) as HTMLElement | null;
}

function $q(selector: string) {
    return addon.data.prefs?.window?.document.querySelector(selector) as HTMLElement | null;
}
...
async function updateMainTable() {
  ...
  const checkedRowIndices = addon.data.prefs.checkedRowIndices || new Set<number>();
  ...
  const baseRenderer: any = (tableHelper as any)?.VirtualizedTable?.makeRowRenderer(getRowData);
  if (typeof baseRenderer === "function") {
    tableHelper.setProp("renderItem", (index, selection, oldElem, columns): Node => {
      ...
      // Highlight Logic
      if (isSelected) {
        rowElem.style.setProperty("background-color", "transparent", "important");
        let bgLayer = rowElem.querySelector(".custom-selection-bg") as HTMLElement;
        if (!bgLayer) {
          ...
          bgLayer.style.backgroundColor = "var(--material-selection-background-color, Highlight)";
      ...
      stickyWrapper.style.backgroundColor = isSelected ? "var(--material-selection-background-color, Highlight)" : "var(--zotero-item-tree-background-color, var(--material-background-color, Canvas))";
      stickyWrapper.style.color = isSelected ? "var(--material-selection-color, HighlightText)" : "var(--zotero-item-tree-color, WindowText)";
  ...
  enableVTableHorizontalScrollSync(addon.data.prefs.window, `${config.addonRef}-table-container`);
}
...
async function updateMineruTable() {
    const renderLock = ztoolkit.getGlobal("Zotero").Promise.defer();
    if (addon.data.prefs?.window == undefined) return;
    const checkedRowIndices = addon.data.prefs.mineruCheckedRowIndices || new Set<number>();
    ...
    const tableHelper = new ztoolkit.VirtualizedTable(addon.data.prefs?.window)
      ...
      .setProp("getRowData", getRowData as any)
      .setProp("getRowString", (index) => (addon.data.prefs?.mineruTokens || [])[index] || "");
    ...
    enableVTableHorizontalScrollSync(addon.data.prefs.window, `${config.addonRef}-mineru-table-container`);
}
...
function bindPrefEvents() {
    ...
    // Delete
    $(`zotero-prefpane-${config.addonRef}-delete`)?.addEventListener("click", () => {
        ...
        const selectedIndices = Array.from(selection?.selected as Set<number> || []);
        
        // Merge checked and selected
        const toDelete = new Set([...Array.from(checked), ...selectedIndices]);
        if (toDelete.size === 0) return;

        const next = (addon.data.prefs?.savedConfigs || []).filter((_, i) => !toDelete.has(i));
    ...
    $(`zotero-prefpane-${config.addonRef}-update`)?.addEventListener("click", () => {
        const tableHelper = addon.data.prefs?.tableHelper;
        const selection = tableHelper?.treeInstance?.selection;
        const selectedIndices = Array.from(selection?.selected as Set<number> || []);
        
        if (selectedIndices.length !== 1) {
            Zotero.alert(win, "Edit Error", "Please select exactly one row to edit.");
            return;
        }
    ...
    // Save (Add new row)
    $(`zotero-prefpane-${config.addonRef}-save`)?.addEventListener("click", () => {
    ...
    // Temperature Slider
    const tempSlider = $(`zotero-prefpane-${config.addonRef}-temperature`) as HTMLInputElement;
    ...
    // Submit
    $(`zotero-prefpane-${config.addonRef}-mineru-submit`)?.addEventListener("click", () => {
        const input = $(`zotero-prefpane-${config.addonRef}-mineru-token`) as HTMLInputElement;
    ...
    // Delete
    $(`zotero-prefpane-${config.addonRef}-mineru-delete`)?.addEventListener("click", () => {
        const tableHelper = addon.data.prefs?.mineruTableHelper;
        const checked = addon.data.prefs?.mineruCheckedRowIndices || new Set<number>();
        const selection = tableHelper?.treeInstance?.selection;
        const selectedIndices = Array.from(selection?.selected as Set<number> || []);
        ...
        const next = (addon.data.prefs!.mineruTokens || []).filter((_, i) => !toDelete.has(i));
        addon.data.prefs!.mineruTokens = next;
    ...
    const openLink = (id: string, url: string) => {
        $(`zotero-prefpane-${config.addonRef}-${id}`)?.addEventListener("click", () => {
...
function getFormData(): SavedConfig {
    const getVal = (id: string) => ($(`zotero-prefpane-${config.addonRef}-${id}`) as HTMLInputElement)?.value || "";
...
function fillForm(c: SavedConfig) {
    const setVal = (id: string, val: string) => {
        const el = $(`zotero-prefpane-${config.addonRef}-${id}`) as HTMLInputElement;
        if (el) el.value = val;
    };
...
function loadSavedConfigs(): SavedConfig[] {
  ...
  try {
    ...
    return parsed.map((v) => normalizeConfig(v)).filter((v): v is SavedConfig => v !== null);
  } catch {
    return [];
  }
}
...
function enableVTableHorizontalScrollSync(win: Window, containerId: string) {
  const container = win.document.getElementById(containerId) as HTMLElement | null;
  if (!container) return;
  ...
  const onScroll = (event: Event) => {
    ...
    const siblings = Array.from(parent.children).filter((node): node is HTMLElement => {
